-- =============================================================================
-- kasroyal_match_integrity.sql — Competitive integrity only.
-- Winner validation, terminal-state rules, distinct player, score/round sanity,
-- cleanup_active_identity_matches_on_terminal, set_finished_at_on_terminal.
-- Do not mix schema, RLS, DELETE, or debug queries here.
-- =============================================================================

-- 1. winner_identity_id must be host, challenger, or NULL
ALTER TABLE public.matches DROP CONSTRAINT IF EXISTS matches_winner_must_be_participant;
...
-- 2. Terminal states require win_reason and finished_at
ALTER TABLE public.matches DROP CONSTRAINT IF EXISTS matches_terminal_requires_reason_and_finished_at;
ALTER TABLE public.matches
...
ALTER TABLE public.matches DROP CONSTRAINT IF EXISTS matches_host_challenger_different;
ALTER TABLE public.matches
...
-- 4. Series score within best_of limits
ALTER TABLE public.matches DROP CONSTRAINT IF EXISTS matches_series_score_limits;
ALTER TABLE public.matches
  ADD CONSTRAINT matches_series_score_limits
  CHECK (
    best_of IN (1, 3, 5)
    AND host_score >= 0 AND challenger_score >= 0
    AND host_score <= (best_of + 1) / 2 AND challenger_score <= (best_of + 1) / 2
  );

-- 5. round_number within [1, best_of]
ALTER TABLE public.matches DROP CONSTRAINT IF EXISTS matches_round_number_limits;
ALTER TABLE public.matches
...
CREATE TRIGGER trg_matches_stamp_finished_at
  ...
  FOR EACH ROW EXECUTE PROCEDURE public.matches_stamp_finished_at();
